...
clone_repository() {
    ...
    if [[ -d "$INSTALL_DIR" ]]; then
        if [[ "$FORCE_RECLONE" == "true" ]]; then
            ...
            if [[ "$response" =~ ^[yY]$ ]]; then
                ...
                cd "$INSTALL_DIR"
                if ! git pull origin release-0.13.0 2>&1 >> "$LOG_FILE"; then
                    error "更新仓库失败"
                    exit $EXIT_DEPENDENCY_FAILED
                fi
                return
    ...
    cd "$INSTALL_DIR"
    info "正在检出 release-0.13.0 分支..."
    if ! git checkout release-0.13.0 2>&1 >> "$LOG_FILE"; then
        error "检出 release-0.13.0 失败"
        exit $EXIT_DEPENDENCY_FAILED
    fi
